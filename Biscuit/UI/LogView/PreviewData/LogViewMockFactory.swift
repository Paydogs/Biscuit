...
struct LogViewMockFactory {
    static func createDummyDomain() -> LogViewDomain {
        let domain = LogViewDomain()
        domain.packets = [.init(id: "Test1111", status: "200", statusColor: Colors.StatusCode.http2xx, method: "GET", methodColor: Colors.RequestMethod.get, url: "http://0.0.0.0:8882/api/v10/login/fingerprint/ios", date: Date.now.formatted()),
    ...
    static func createDummyHandler() -> LogViewEventHandling {
        return DummyHandler()
    }
}

private extension LogViewMockFactory {
